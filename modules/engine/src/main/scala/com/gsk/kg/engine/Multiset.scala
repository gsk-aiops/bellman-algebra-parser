...
/** A [[Multiset[A]] ], as expressed in SparQL terms.
  ...
  * @param bindings
  *   the variables this Multiset[A] expose
  * @param relational
  *   the underlying data that the Multiset[A] contains
  */
final case class Multiset[A: Relational](
  ...
  /** Adds graph binding and column to the Multiset[A] (with default graph as
    * the default value)
    */
